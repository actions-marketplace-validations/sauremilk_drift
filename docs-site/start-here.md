# Evaluate Drift

Three paths depending on what you need right now.

## I want to try it (2 minutes)

1. [Quick Start](getting-started/quickstart.md) — install, run, read your first findings.

## I want to check the evidence first

1. [Trust and Evidence](trust-evidence.md) — conservative claims, known limits, raw artifacts.
2. [Example Findings](product/example-findings.md) — 5 concrete findings with code, result, and fix path.
3. [Benchmark Study](study.md) — methodology, per-signal precision, mutation recall.

## I want to plan a team rollout

1. [Team Rollout](getting-started/team-rollout.md) — 4-phase adoption from report-only to gated CI.
2. [Integrations](integrations.md) — GitHub Action, GitLab CI, pre-commit, MCP, Python API.
3. [Stability and Release Status](stability.md) — what the Beta classifier does and does not mean.